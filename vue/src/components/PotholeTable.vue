<template>
  <div class="pothole-table">
    ...
    <div v-for='pothole in $store.state.potholes' v-bind:key='pothole.id'>
        <div v-if='pothole.isReviewed || $store.state.user.role=="admin"' class="cards">
        <router-link 
        ...
        <p class="id">ID: {{pothole.id}} </p> </router-link>
            ...
            <p class="reviewed" v-if='$store.state.user.role=="admin"'>Reviewed: {{pothole.isReviewed == true ? 'Yes' : 'No'}}</p>
