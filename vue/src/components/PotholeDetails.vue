<template>
    <div>
        <div v-if="pothole">
            ...
            <p class="reviewed">Reviewed: {{pothole.isReviewed == true ? 'Yes' : 'No'}}</p>
            <p class="inspected">Inspected: {{pothole.isInspected == true ? 'Yes' : 'No'}}</p>
            <p class="inspection-date">Inspection Date: {{pothole.inspectionDate.substring(0,10)}}</p>
            <p class="repaired">Repaired: {{pothole.isRepaired == true ? 'Yes' : 'No'}}</p>
            <p class="repair-date">Repair Date: {{pothole.repairDate.substring(0,10)}}</p>
        </div>
    </div>
</template>
